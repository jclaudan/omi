...
if (-not (Test-Path ".env")) {
    ...
    Write-Host "✅ Created .env - please edit it with your settings" -ForegroundColor Green
    Write-Host ""
    Write-Host "Edit .env and run again:" -ForegroundColor Cyan
...
if ($mode -eq "tunnel") {
    ...
    $token = (Get-Content .env | Select-String "CLOUDFLARE_TUNNEL_TOKEN=" | ForEach-Object { $_ -replace "CLOUDFLARE_TUNNEL_TOKEN=", "" }).ToString().Trim()

    if ([string]::IsNullOrEmpty($token)) {
        Write-Host "❌ CLOUDFLARE_TUNNEL_TOKEN not set in .env!" -ForegroundColor Red
        Write-Host "💡 Add your token to .env and try again" -ForegroundColor Yellow
        exit 1
    }
